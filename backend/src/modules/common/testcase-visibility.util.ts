...
function sanitizeCodingConfigForClient(
  ...
  const sanitized: any = {
    ...codingConfig,
    testCases: sanitizeTestCasesForClient(
  ...
  if (sanitized.templates && typeof sanitized.templates === 'object') {
    const templateEntries = Object.entries(sanitized.templates).map(
      ([lang, template]: [string, any]) => {
...
export function sanitizeQuestionForClient(
  ...
  if (Array.isArray(sanitized.mcqOptions)) {
    sanitized.mcqOptions = sanitizeMcqOptionsForClient(
      sanitized.mcqOptions,
      includeSensitive,
    );
  }

  if (Array.isArray(sanitized.options)) {
    sanitized.options = sanitizeMcqOptionsForClient(
      sanitized.options,
      includeSensitive,
    );
  }
  ...
  if (
    sanitized.mcq &&
    typeof sanitized.mcq === 'object' &&
    Array.isArray(sanitized.mcq.options)
  ) {
    sanitized.mcq = {
      ...sanitized.mcq,
      options: sanitizeMcqOptionsForClient(
        sanitized.mcq.options,
        includeSensitive,
      ),
    };
  }
  ...
  if (sanitized.codingConfig) {
    sanitized.codingConfig = sanitizeCodingConfigForClient(
      sanitized.codingConfig,
      includeSensitive,
    );
  }

  if (sanitized.coding) {
    sanitized.coding = sanitizeCodingConfigForClient(
      sanitized.coding,
      includeSensitive,
    );
  }
  ...
  if (Array.isArray(sanitized.testCases)) {
    ...
    sanitized.testCases = sanitizeTestCasesForClient(
      sanitized.testCases,
      showTestCases,
      includeSensitive,
    );
...
export function sanitizeQuestionsPayloadForClient(
      ...
      if (item && typeof item === 'object' && Array.isArray(item.questions)) {
        return {
          ...
          questions: item.questions.map((q: any) =>
            sanitizeQuestionForClient(q, includeSensitive),
          ),
        };
      }
  ...
  if (typeof questionsPayload === 'object') {
    const sanitizedEntries = Object.entries(questionsPayload).map(
...
function sanitizeMcqOptionsForClient(
  options: any[],
  includeSensitive: boolean,
): any[] {
