...
export class StudentService {
  ...
  async getStats(userId: string) {
    ...
    const sessionsAgg = await this.prisma.examSession.aggregate({
      where: {
        userId,
        score: { not: null },
    ...
    const averageScore =
      sessionsAgg._avg.score !== null ? Math.round(sessionsAgg._avg.score) : 0;

    const stats = {
      completedModules: user.unitSubmissions.length,
      averageScore,
