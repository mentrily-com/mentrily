...
export class StudentService {
  ...
  async getStats(userId: string) {
    ...
    // ⚡ Bolt: Optimized to use DB-level aggregation instead of fetching rows and computing in memory
    // Calculate average score - only from published results
    const result = await this.prisma.examSession.aggregate({
      where: {
        userId,
        score: { not: null },
    ...
    const averageScore = result._avg.score ? Math.round(result._avg.score) : 0;
