  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  async onModuleInit() {
    const eagerConnect =
      String(process.env.PRISMA_EAGER_CONNECT || 'false').toLowerCase() ===
      'true';
    if (!eagerConnect) {
      console.log(
        '[PrismaService] Skipping eager DB connect on startup (PRISMA_EAGER_CONNECT=false).',
      );
      return;
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (!isRetryableConnectError(error)) {
          console.error(
            '[PrismaService] Non-retryable DB connection error detected. Failing fast.',
          );
          throw error;
        }
        ...
        console.error(
          `[PrismaService] Connect attempt ${attempt}/${maxAttempts} failed. Retrying in ${delayMs}ms...`,
        );
