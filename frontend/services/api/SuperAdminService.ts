...
export const SuperAdminService = {
    ...
    async updateOrganizationPlan(id: string, plan: 'FREE' | 'STARTER' | 'PRO' | 'ENTERPRISE') {
        try {
            const res = await authFetch(`/super-admin/organizations/${id}/plan`, {
                method: 'PATCH',
                body: JSON.stringify({ plan }),
            });
            if (!res.ok) throw new Error('Failed to update organization plan');
            return await res.json();
        } catch (error) {
            console.error('[SuperAdminService] Error', error);
            throw error;
    ...
    async updateOrganizationLimits(
        ...
        try {
            const res = await authFetch(`/super-admin/organizations/${id}/limits`, {
                method: 'PATCH',
                body: JSON.stringify(data),
            });
            if (!res.ok) throw new Error('Failed to update organization limits');
            return await res.json();
        } catch (error) {
            console.error('[SuperAdminService] Error', error);
            throw error;
    ...
    async transferUserToOrganization(userId: string, targetOrgId: string) {
        try {
            const res = await authFetch('/super-admin/transfer-user', {
                method: 'POST',
                body: JSON.stringify({ userId, targetOrgId }),
            });

            if (!res.ok) {
                const errorData = await res.json().catch(() => ({}));
                throw new Error(errorData.message || 'Failed to transfer user');
            }
