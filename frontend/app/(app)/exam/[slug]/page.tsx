...
const ExamSubmitView = dynamic(() => import('@/app/components/ExamSubmitView'), {
    ssr: false,
});
const ExamFeedbackView = dynamic(() => import('@/app/components/ExamFeedbackView'), {
    ssr: false,
});
const ExamSuccessView = dynamic(() => import('@/app/components/ExamSuccessView'), {
    ssr: false,
});
...
import { useNetworkMonitor, useNetworkOnlineStatus } from '@/hooks/useNetworkMonitor';
...
interface ExamCountdownTimerProps {
    ...
    isPaused: boolean;
    onTimeUp: () => void;
    onWarning: (message: string, title: string, duration: number) => void;
}
...
const ExamCountdownTimer = React.memo(function ExamCountdownTimer({
    ...
    useEffect(() => {
        if (secondsLeft === null || isPaused) return;
    ...
    useEffect(() => {
        ...
        if (secondsLeft === 300 && !fiveMinWarningShownRef.current) {
...
export default function PublicExamPage() {
    ...
    const isOnline = useNetworkOnlineStatus();
    ...
    const [isWebcamMinimized, setIsWebcamMinimized] = useState(() => {
            ...
            try {
                ...
                const resolveCurrentUser = async () => {
                    ...
                    for (let attempt = 0; attempt < attempts; attempt += 1) {
                        // Fast path: use session cache on first attempt (0ms resolution).
                        // Only bypass cache on retries if initial token wasn't ready yet.
                        const sessionUser = await AuthService.checkSession(true, { bypassCache: attempt > 0 });
    ...
    const handleQuestionSelect = useCallback(
        (sectionId: string, questionId: string | number, force = false) => {
        ...
        [sections, sessionId, saveAnswer, writeLocalDraft],
    );
    ...
    }, [sections, currentSectionId, currentQuestionId, handleQuestionSelect]);
    ...
    // Track review and submitted markers without triggering effects on every text/code edit
    const statusSignature = useMemo(() => {
        const keys = Object.keys(userAnswers)
            .filter((k) => k.startsWith('_rev_') || k.startsWith('_submitted_'))
            .sort();
        return keys.map((k) => `${k}:${userAnswers[k]}`).join('|');
    }, [userAnswers]);

    // Automatically update question statuses in the sidebar when review/submitted flags change
    useEffect(() => {
        ...
        setSections((prev) => {
            let hasAnyStatusChange = false;
            const nextSections = prev.map((s) => {
                let sectionChanged = false;
                const nextQuestions = s.questions.map((q: any) => {
                    const nextStatus = getDynamicStatus(q, userAnswersRef.current);
                    if (q.status !== nextStatus) {
                        sectionChanged = true;
                        hasAnyStatusChange = true;
                        return { ...q, status: nextStatus };
                    }
                    return q;
                });
                return sectionChanged ? { ...s, questions: nextQuestions } : s;
            });
    ...
    }, [currentQuestionId, sessionId, slug, writeLocalDraft, saveReviewStatus]);
    ...
    }, [sections, currentSectionId, currentQuestionId]);
    ...
    const isCurrentSectionSubmitted = useMemo(() => {
        return sections.find((s) => s.id === currentSectionId)?.status === 'submitted';
    }, [sections, currentSectionId]);

    const areAllSectionsSubmitted = useMemo(() => {
        return sections.length > 0 && sections.every((s) => s.status === 'submitted');
    }, [sections]);
    ...
    const examConfig: ExamConfig = useMemo(
        () => ({
            ...
            centerContent:
                isSubmitModalOpen || isCurrentSectionSubmitted ? (
            ...
            rightContent: (
                <div className="flex shrink-0 items-center gap-2 sm:gap-4">
                    <ExamCountdownTimer
                        ...
                        isPaused={isFeedbackMode || isSuccessMode}
                        onTimeUp={submitFullExam}
                        onWarning={warning}
                    />
                    ...
                    {/* WiFi Signal Icon with Tooltip (isolated to leaf component to eliminate 10s root page re-renders) */}
                    <ExamNetworkIndicator />
        ...
        [
            ...
            user?.profilePicture,
            user?.imageUrl,
            examMetadata?.profilePicture,
            windowFocus.in,
            windowFocus.out,
            isSubmitModalOpen,
            isCurrentSectionSubmitted,
            areAllSectionsSubmitted,
            timeLeft,
            isFeedbackMode,
            isSuccessMode,
            submitFullExam,
    ...
    }, [setElectronStrictMode, getCourseReturnHref]);
    ...
    return (
        ...
        >
            ...
            {/* AI Proctoring Webcam (Clean Preview with Minimization) */}
            {!isFeedbackMode && !isSuccessMode && isAiProctoringEnabled && (
                <div className="fixed top-16 right-3 sm:top-auto sm:bottom-24 sm:right-6 z-[90] pointer-events-none">
                    ...
                    >
                        ...
                        {isWebcamMinimized ? (
                            <div className="flex items-center gap-2 select-none text-xs font-semibold text-white">
                            ...
                            <>
                                ...
                                {!isModelLoaded && <div className="absolute inset-0 bg-black/40 pointer-events-none" />}
                            </>
                        )}
                    </div>
                </div>
