...
export default function ModulePage({ params: paramsPromise }: { params: Promise<{ slug: string }> }) {
    ...
    const parseScheduleDate = (value: unknown) => {
    ...
    const calculateDuration = (start: Date | null, end: Date | null) => {
        ...
        if (!Number.isFinite(diffMs) || diffMs <= 0) return 'No schedule';
    ...
    const togglePerfExpand = (id: string) => {
        setExpandedPerfIds((prev) => (prev.includes(id) ? prev.filter((i) => i !== id) : [...prev, id]));
    };
        ...
        const extractCodeBlocksFromHtml = (html: string) => {
            ...
            while ((m = codeRegex.exec(html)) !== null) {
                ...
                blocks.push({
                    id: `desc-code-${i++}`,
                    type: 'code',
                    codeConfig: { languageId: language.toLowerCase(), initialCode: decodeHtmlEntities(code.trim()) },
                });
            }
            return blocks;
        };
            ...
            readingContent = content.readingContent.map((b: any, idx: number) => {
                if ((b.type || '').toLowerCase() === 'code' || b.code || b.codeConfig) {
                    ...
                    return {
                        id: b.id || `reading-${idx}`,
                        type: 'code',
                        codeConfig: { languageId: language, initialCode },
                    };
                }
        ...
        const mapped = {
            ...
            codingConfig: (function () {
                ...
                return {
                    ...
                    initialCode: primaryTemplate.initialCode || primaryTemplate.body || cc.initialCode || cc.code || '',
            ...
            mcqOptions: Array.isArray(content.options)
                ? content.options.map((o: any) => ({ id: o.id, text: o.text }))
                : Array.isArray(content.mcqOptions)
                  ? content.mcqOptions.map((o: any) => ({ id: o.id, text: o.text }))
                  : [],
            readingContent,
            notebookConfig: (function () {
                const nb = content.notebookConfig || content.notebook || undefined;
                if (!nb) return undefined;
                return { initialCode: nb.initialCode || nb.initial_code || '', language: nb.language || 'python' };
            })(),
    ...
    const performanceItems = selectedModule
        ? [
              ...
              ...(course?.tests?.flatMap((t: any) =>
                  flattenTestQuestions(t).map((q: any, qi: number) => ({
                      id: `T-${t.id}-${q.id}`,
        ...
        async function loadCourse() {
            ...
            try {
                ...
                const [data, progress, examStatus] = await Promise.all([
                    ...
                    StudentService.getExamStatus(slug).catch(() => null),
                ]);
                if (!mounted) return;
                setCourse({
                    ...data,
                    linkedExam: examStatus?.linkedExam || data?.linkedExam,
                });
    ...
    const handleReturnToLearnerDashboard = React.useCallback(() => {
    ...
    const progressPercent =
        progressData && progressData.totalUnits > 0
            ? Math.round((progressData.completedUnitIds.length / progressData.totalUnits) * 100)
            : 0;

    const linkedExam = course.linkedExam
        ? (() => {
              const requiredPercent = Number(course.linkedExam.requiredPercent ?? course.examUnlockThreshold ?? 100);
              return {
                  ...
                  isActive: Boolean(course.linkedExam.isActive),
                  isUnlocked: progressPercent >= requiredPercent,
                  requiredPercent,
                  passingPercentage: course.linkedExam.passingPercentage,
                  lastAttempt: course.linkedExam.lastAttempt,
                  passed: course.linkedExam.passed,
                  attemptsUsed: course.linkedExam.attemptsUsed,
                  attemptsRemaining: course.linkedExam.attemptsRemaining,
                  nextAttemptAvailableAt: course.linkedExam.nextAttemptAvailableAt,
              };
          })()
        : null;
        ...
        <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans selection:bg-[var(--brand-light)] selection:text-[var(--brand-dark)]">
            ...
            <div className="sticky top-[56px] sm:top-[61px] z-40 bg-white border-b border-slate-200/60 shadow-sm transition-all duration-300">
                <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 py-3.5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex w-full flex-col items-stretch gap-3 sm:w-auto sm:flex-row sm:items-center sm:justify-between sm:gap-6">
                        {linkedExam && (
                            <StudentExamCard
                                exam={linkedExam}
                        ...
                        <button
                            className="p-2 rounded-xl text-slate-400 hover:text-[var(--brand)] hover:bg-[var(--brand-lighter)] transition-all active:scale-95"
                            aria-label="Share"
                        >
                            <svg
                                width="20"
                ...
                {activeTab === 'learning' ? (
                    <div className="space-y-10">
                        <div className="relative group/carousel">
                            ...
                            >
                                {course.modules.map((m: any, i: number) => {
                                    ...
                                    const completedCount = moduleUnits.filter((u: any) =>
                                        progressData?.completedUnitIds.includes(u.id),
                                    ).length;
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <div
                                                    className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black transition-colors ${activeModuleIndex === i ? 'bg-[var(--brand-light)] text-[var(--brand)]' : 'bg-slate-50 text-slate-400'}`}
                                                >
                                                    {isCompleted ? (
                                                        <svg
                                                            width="14"
                                            ...
                                            <h4
                                                className={`font-black text-[15px] leading-snug line-clamp-2 ${activeModuleIndex === i ? 'text-slate-900' : 'text-slate-600'}`}
                                            >
                                                {m.title}
                                            </h4>
                                            ...
                                            <div className="flex items-center justify-between text-[10px] font-black text-slate-400 border-t border-slate-50 pt-3">
                                                ...
                                                <div className="w-12 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                                    <div
                                                        className={`h-full rounded-full bg-[var(--brand)] transition-all opacity-30 ${activeModuleIndex === i ? 'opacity-100' : ''}`}
                                                        style={{ width: `${progressPercent}%` }}
                                                    ></div>
                                                </div>
                                            </div>
                        ...
                        <div className="space-y-3">
                            <h2 className="text-lg font-black text-slate-800 tracking-tight ml-2 mb-4">
                                Unit Curriculum
                            </h2>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                <div
                                                    className={`text-xs font-black w-6 text-center transition-colors text-slate-300`}
                                                >
                                                    {isCompleted ? (
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <h3
                                                        className={`text-[15px] font-bold transition-colors text-slate-700`}
                                                    >
                                            ...
                                            <div className="flex items-center gap-3 sm:gap-6">
                                                <div className="flex items-center gap-3 opacity-100 transition-all sm:translate-x-4 sm:opacity-0 sm:group-hover:translate-x-0 sm:group-hover:opacity-100">
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                    ...
                    <div className="space-y-10">
                        ...
                        <div className="relative group/carousel">
                            ...
                            >
                                {course.tests?.map((t: any, i: number) => {
                                    const now = new Date();
                                    const start = parseScheduleDate(t.startDate);
                                    const end = parseScheduleDate(t.endDate);
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <div
                                                    className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black transition-colors ${activeTestIndex === i ? 'bg-[var(--brand-light)] text-[var(--brand)]' : 'bg-slate-50 text-slate-400'}`}
                                                >
                                                    {i + 1}
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <h4
                                                    className={`font-black text-[15px] leading-snug line-clamp-1 ${activeTestIndex === i ? 'text-slate-900' : 'text-slate-600'}`}
                                                >
                                                    {t.title}
                                                </h4>
                        ...
                        <div className="space-y-3">
                            {(() => {
                                ...
                                const now = new Date();
                                const start = parseScheduleDate(activeTest.startDate);
                                const end = parseScheduleDate(activeTest.endDate);
                                const isLocked = !!start && now < start;
                                const isExpired = !!end && now > end;
                                ...
                                if (isLocked) {
                                    return (
                                        <>
                                            ...
                                            <div className="p-6 text-center bg-slate-50/50 rounded-[24px] border border-dashed border-slate-200 sm:p-12">
                    ...
                    <div className="space-y-10 animate-fade-in">
                        ...
                        <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                            <div className="hidden items-center px-8 py-4 bg-slate-50/50 border-b border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-400 md:flex">
                                ...
                                {performanceItems.map((q) => (
                                    <div key={q.id} className="bg-white">
                                        <div
                                            ...
                                            className="flex flex-col gap-3 px-4 py-4 hover:bg-slate-50/30 cursor-pointer transition-colors group md:flex-row md:items-center md:px-8 md:py-5"
                                        >
                                            <div className="flex-1 flex min-w-0 items-center gap-4">
                                                <span className="text-[11px] font-black text-slate-300 w-4">
                                                    {q.displayId}.
                                                </span>
                                                <span className="text-xs font-bold text-slate-700 group-hover:text-[var(--brand)] transition-colors">
                                                    {q.title}
                                                </span>
                                            </div>
                                            <div className="w-fit md:w-32 text-left md:text-center text-[10px] font-black text-slate-400 uppercase tracking-tighter">
                                                {q.type}
                                            </div>
                                        ...
                                        {expandedPerfIds.includes(q.id) && (
                                            <div className="px-4 pb-6 pt-2 animate-in slide-in-from-top-4 duration-300 sm:px-8 sm:pb-8">
                                                {q.attempts.length > 0 ? (
                                                    <>
                                                        <div className="flex flex-wrap items-center justify-center gap-3 mb-6 sm:gap-6 sm:mb-8">
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    setPerfFilter('all');
                                                        ...
                                                        <div className="max-w-4xl mx-auto border border-slate-50 rounded-xl bg-slate-50/30 px-3 py-4 sm:px-6">
                                                            <div className="hidden items-center text-[9px] font-black uppercase text-slate-400 mb-4 border-b border-slate-100 pb-2 sm:flex">
                                                                ...
                                                                {q.attempts.map((attempt: any, idx: number) => (
                                                                    <div
                                                                        ...
                                                                        onClick={(e) => {
                                                                            ...
                                                                            const unitId = q.id.startsWith('L-')
                                                                                ? q.id.substring(2)
                                                                                : q.id.split('-').pop();
                                                                            router.push(
                                                                        ...
                                                                        className="grid grid-cols-1 gap-2 text-[11px] font-bold text-slate-600 hover:bg-slate-100 p-2 rounded-lg cursor-pointer transition-colors sm:flex sm:items-center"
                                                                    >
                                                                        <div className="flex-1 text-center font-mono opacity-80">
                                                                            {attempt.date}
                                                                        </div>
                                                                        <div
                                                                            className={`flex-1 text-center ${attempt.status === 'success' ? 'text-emerald-500' : 'text-rose-400'}`}
                                                                        >
                                                                            {attempt.testCases}
                                                                        </div>
                                                                        <div className="flex-1 text-center">
                                                                            <span
                                                                                className={`px-2 py-0.5 rounded-md ${attempt.status === 'success' ? 'text-emerald-500' : 'text-rose-500'}`}
                                                                            >
                                                                                {attempt.status}
                                                                            </span>
                                                                        </div>
