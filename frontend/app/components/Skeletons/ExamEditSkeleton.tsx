/**
 * Purpose-built skeleton for `/dashboard/creator/exams/[id]/edit`, both
 * while the exam record itself is loading and while the `ExamBuilder`
 * studio is being dynamically imported afterward. The previous fallback
 * was the generic `DashboardSkeleton type="form"`, a centered card of
 * label+input pairs -- nothing like the real studio, which fills the
 * viewport with a sticky toolbar over a fixed left sidebar (stats strip,
 * tab toggle, section list) next to a scrolling main canvas of stacked
 * cards. This mirrors that toolbar/sidebar/canvas rhythm and proportions
 * rather than the exact controls, so the two loading stages resolve into
 * the same shape instead of popping between a boxed form and the studio.
 */
export default function ExamEditSkeleton() {
    return (
