/**
 * Purpose-built skeleton for the super-admin course edit page
 * (`/dashboard/super-admin/organizations/[id]/courses/[courseId]/edit`),
 * which renders `CourseEditor` -> `CourseBuilder` once the course loads.
 * Previously this route used the generic `DashboardSkeleton type="form"`,
 * a plain white card with a 2x2 field grid that shares nothing with the
 * builder's real shape: a sticky header (back arrow, title input, status
 * pill, save button), a units/tests stats strip, a tab bar, and a two-
 * column body (a unit list sidebar beside a details card with labeled
 * fields), so the page visibly reflowed once the course data and builder
 * mounted. Mirrors that real chrome and column split rather than exact
 * pixel content.
 */
