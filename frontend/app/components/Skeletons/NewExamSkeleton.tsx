/**
 * Purpose-built skeleton for `/dashboard/creator/exams/new`, which mounts
 * the `ExamBuilder` studio (dynamically imported, no SSR) as soon as the
 * page loads. The previous fallback was the generic `DashboardSkeleton
 * type="form"`, a centered card of label+input pairs -- nothing like the
 * real studio, which fills the viewport with a sticky toolbar over a fixed
 * left sidebar (stats strip, tab toggle, section list) next to a scrolling
 * main canvas of stacked cards. This mirrors that toolbar/sidebar/canvas
 * rhythm and proportions rather than the exact controls, so the builder
 * chrome resolves in place instead of the page reflowing around it.
 */
export default function NewExamSkeleton() {
    return (
