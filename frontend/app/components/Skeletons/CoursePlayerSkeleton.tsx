...
export default function CoursePlayerSkeleton({
    ...
    fillHeight = true,
}: CoursePlayerSkeletonProps) {
    const rootHeight = fillHeight ? 'h-full min-h-0' : 'h-screen';
    const sidebarWidth = sidebarCollapsed ? 'w-16' : 'w-[300px]';

    return (
        <div className={`${rootHeight} bg-white flex flex-col overflow-hidden font-sans`}>
            {/* Top Full-Width Navbar for Exam Mode */}
            {isExamMode && (
                <div className="h-16 border-b border-slate-100 bg-white flex items-center justify-between px-6 shrink-0 z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="w-28 h-5 rounded-md bg-slate-200 animate-pulse" />
                    </div>
                    <div className="flex items-center gap-3">
            ...
            <div className="flex-1 flex overflow-hidden relative">
                {/* Left Sidebar Skeleton (UnitSidebar or ExamSidebar) */}
                {hasSidebar && (
                    <div className={`${sidebarWidth} h-full bg-slate-50 border-r border-slate-200 hidden md:flex flex-col shrink-0 transition-all`}>
                        {sidebarCollapsed ? (
                            <div className="p-3 space-y-4 flex flex-col items-center">
                                <div className="w-8 h-8 rounded-xl bg-slate-200 animate-pulse mt-2" />
                                {[1, 2, 3, 4, 5].map((i) => (
                                    <div key={i} className="w-8 h-8 rounded-full bg-slate-200/80 animate-pulse" />
                                ))}
                            </div>
                        ) : (
                            <>
                                <div className="p-6 border-b border-slate-200 space-y-3">
                                    ...
                                    <div className="w-48 h-6 bg-slate-300 rounded-lg animate-pulse" />
                                </div>
                ...
                <div className="flex-1 flex flex-col bg-white overflow-hidden relative">
                    {/* Header/Nav inside course player (only when NOT in exam mode, since exam has top full navbar) */}
                    {!isExamMode && (
                    ...
                    <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                        ...
                        <div className="flex-1 lg:max-w-[50%] p-8 lg:p-12 overflow-hidden border-b lg:border-b-0 lg:border-r border-slate-100">
                            <div className="space-y-6 max-w-2xl mx-auto lg:mx-0">
                                ...
                                <div className="w-full h-2 rounded-full bg-slate-100 animate-pulse my-8" />
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="w-full h-4 bg-slate-100 rounded animate-pulse mt-4" />
                                    <div className="w-4/5 h-4 bg-slate-100 rounded animate-pulse" />
                        ...
                        <div className="flex-1 bg-slate-50 relative flex flex-col">
                            ...
                            <div className="h-12 bg-slate-100 border-b border-slate-200 flex items-center px-4 gap-2 shrink-0">
                                <div className="w-24 h-8 bg-white border border-slate-200 rounded-md animate-pulse" />
                                <div className="w-24 h-8 bg-slate-200/50 rounded-md animate-pulse" />
                            </div>
                            {/* Editor Body Fake */}
                            <div className="flex-1 p-6 space-y-3">
                                <div className="w-32 h-4 bg-slate-200/50 rounded animate-pulse" />
                                <div className="w-48 h-4 bg-slate-200/50 rounded animate-pulse pl-4" />
                                <div className="w-40 h-4 bg-slate-200/50 rounded animate-pulse pl-8" />
                                <div className="w-24 h-4 bg-slate-200/50 rounded animate-pulse pl-4" />
                                <div className="w-16 h-4 bg-slate-200/50 rounded animate-pulse" />
                            </div>
                            {/* Action Bar Fake */}
                            <div className="h-16 bg-white border-t border-slate-200 flex items-center justify-between px-6 shrink-0">
                                <div className="w-20 h-10 bg-slate-100 rounded-xl animate-pulse" />
