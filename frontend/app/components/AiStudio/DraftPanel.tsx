...
import { AlertTriangle, ExternalLink, Loader2, Lock, Sparkles, X } from 'lucide-react';
...
export default function DraftPanel({
    ...
    const write = async () => {
        ...
        if (usage && !usage.features.aiExams) {
            onLocked(`Writing full ${noun}s with AI is available on the Starter plan and above. Your outline is kept here.`, 'Available on Starter plan');
            return;
        }
        setBusy('write');
        try {
    ...
    const saveDraft = async () => {
        ...
        try {
            ...
            setSaved({ id: res.id, kind: draft.kind });
        } catch (err) {
            onError(err);
        } finally {
            setBusy(null);
    ...
    const title =
        draft?.title ?? blueprint?.title ?? job?.brief?.topic ?? (job?.kind === 'quiz' ? 'Quiz' : 'Generation');
    ...
    return (
        <aside className="flex h-full min-h-0 flex-col bg-white">
            <header className="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
                <div className="min-w-0">
                    <p className="text-xs text-slate-500">
                        {draft ? `${noun === 'exam' ? 'Exam' : 'Course'} draft` : blueprint ? 'Outline' : 'Generating'}
                    </p>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
                {!job ? (
                    ...
                    <div className="space-y-4">
                        ...
                        <BlueprintEditor blueprint={blueprint} onChange={setBlueprint} maxQuestions={maxQuestions} />
                    </div>
                ) : draft ? (
                    <div className="space-y-4">
                        {saved ? (
                        ...
                        {confirmReplace && (
                            <div className="space-y-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                                <p>The {noun} builder has an unsaved new {noun}. Opening this draft replaces it.</p>
            ...
            {job?.status === 'completed' && (blueprint || draft) && (
