...
export default function AdminExamsView({ basePath = '/dashboard/creator', organizationId }: AdminExamsViewProps) {
    ...
    const orgPermissions = userData?.features || {
        canCreateExams: true,
    ...
    const formatSchedule = (start?: string | Date | null, end?: string | Date | null) => {
        const startLabel = formatDate(start);
        const endLabel = formatDate(end);
        if (startLabel === '—' && endLabel === '—') return 'Not scheduled';
        ...
        <div className="animate-fade-in font-sans pb-10">
            ...
            <div className="flex gap-8 border-b border-slate-100 mb-10">
                ...
                >
                    Total Examinations
                    {activeTab === 'exams' && (
                        <div className="absolute bottom-0 left-0 right-0 h-1 bg-[var(--brand)] rounded-full animate-fade-in" />
                    )}
                </button>
                <button
            ...
            />
            {viewingExam && (
                <ExamDetailsModal exam={viewingExam} onClose={() => setViewingExam(null)} userRole="admin" />
            )}
            {enrollingCourse && (
                <EnrollmentModal
