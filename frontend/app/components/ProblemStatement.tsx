...
export default function ProblemStatement({
    ...
    return (
        <div className="flex h-full min-h-0 flex-col bg-white relative" data-element-id="starter-question-prompt">
            ...
            {/* Bottom Navigation (Fixed Footer) -- shared between exam mode and
                course questions now, so both get the same Previous/Next
                buttons in the same place instead of course questions having
                a separate top-of-panel arrow pair. Renders nothing when the
                caller supplies neither handler (e.g. the standalone
                single-question playground). */}
            <QuestionNavFooter onPrevious={onPrevious} onNext={onNext} />
        </div>
    );
}
