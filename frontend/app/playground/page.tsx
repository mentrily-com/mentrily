...
export default function PlaygroundPage() {
    ...
    const handleRun = async () => {
        ...
        try {
            ...
            setTerminalOutput((prev) => [...prev, `Error: Failed to execute code.`, String(error)]);
        } finally {
            setIsRunning(false);
        }
    };
    ...
    return (
        <div className="w-full h-full min-h-0 flex flex-col overflow-hidden font-sans rounded-2xl border bg-white" style={{ borderColor: 'var(--color-border-subtle)', boxShadow: 'var(--shadow-sm)' }}>
            <div className="h-14 shrink-0 border-b px-5 flex items-center justify-between" style={{ borderColor: 'var(--color-border-subtle)', backgroundColor: 'var(--color-bg-subtle)' }}>
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <p className="text-sm font-semibold" style={{ color: 'var(--color-text-primary)' }}>Code Playground</p>
                        <p className="text-[11px]" style={{ color: 'var(--color-text-secondary)' }}>Run and iterate instantly</p>
                    </div>
                </div>
            </div>
                                    ...
                                    <div className="flex items-center gap-1 h-full">
                                        ...
                                        {tabs.map((tab) => (
                                            <div
                                                ...
                                                className={`group px-6 h-full flex items-center text-[12px] font-bold transition-all border-b-2 relative cursor-pointer ${activeTabId === tab.id
                            ...
                            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full animate-in zoom-in duration-200">
                                ...
                                <p className="text-slate-500 text-sm mb-8 leading-relaxed">
                                    ...
                                    <span className="font-bold text-slate-700">
                                        &ldquo;{tabs.find((t) => t.id === showDeleteConfirm)?.name}&rdquo;
                                    </span>
